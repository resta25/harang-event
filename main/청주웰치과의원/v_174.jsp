...
.form .description.orage-box{display: flex; justify-content: flex-start; padding-top:3%; padding-bottom: 3%; background-color: #f1fbff;}
...
.form .question_box .question, .user_name, .user_phone {display: flex; align-items: center; column-gap: 2rem; width: 90%; margin: 0 auto; margin-bottom: 2%; padding: 1.7rem 1.55rem; border-radius: 20px; border: solid 1px #bebebe; background-color: #fff;}
.selected_area{background-color:#000; padding:3% 5%;}
.q_container {width: 47%;}
.q_select {display: flex; align-items: center; column-gap: 1.3rem;}
.q_box {}
.q_box, .legend {width: max-content; flex-shrink: 0; padding: 1.4rem 2.3rem; font-size: 3.55rem; letter-spacing: -1.78px; border-radius: 999px; border: solid 3px #1f282d; background-color: #fff; line-height: 1; font-weight: 800;}

/* 설문값, 동의, 미동의 버튼 공통 */
.video-area {}
.video-area video {display: block; width: 100%;}
...
.form .question_box .q_select label {flex-shrink: 0; font-weight: 800; font-size: 2.7rem; padding: 1.8rem 2.2rem; line-height: 1; letter-spacing: -1.35px; color: #fff;  border-radius: 1.5rem;  background-color: #1f282d;}
.form .question_box .q_select label.on {background-color: #00b4ff; color:#fff;}
...
/* 이름, 연락처 공통 */
.form .user_info input {  border-radius: 1rem;
  border: solid 1px #68caff;
  background-color: #fffbd7; font-size: 2.7rem;     padding: 1.8rem 2.2rem;}
...
.form .agBox .disagree {display: inline-block; flex-shrink: 0; font-weight: 800; font-size: 2.7rem; padding: 1.8rem 2.2rem; line-height: 1; letter-spacing: -1.35px; text-align: right; color: #fff; border-radius: 1.5rem; background-color: #1f282d;}

.form .agBox.on label{background-color:#00b4ff;}
...
.question_container {background: #f1fbff ; padding: 4% 0;}

.form .agreement {font-size: 1.6rem;}

.form .description {margin-top: 1rem;}
.form .description p, .form .description span {font-weight: 400; font-size: 1.4rem;}
...
@media screen and (max-width: 500px){
    ...
    .q_container {width: 33%;}
    .form .question_box .q_select label {padding: 1.8rem 1.7rem;}
    .q_box, .legend {font-size: 2.5rem;}
}
    ...
    <div class="form">
        ...
        <div class="question_container">
            <div class="question_box">
				...
				<div class="question">
                    <div class="q_container">
                        <div class="q_box">고민사항</div>
                    </div>
					<div class="q_select">
						<label>출혈<input type="radio" name="tadd1" value="출혈"></label>
						<label>통증<input type="radio" name="tadd1" value="통증"></label>
				...
				<div class="agBox">
					<div class="question">
						<a href="#" class="btn-agreement">
							<div class="q_box">개인정보 수집 · 이용</div>
						</a>
				...
				<div class="formContents">	
					...
					<!-- <div class="img-area result">
						...
						<img src="//static.harang-event.com/event/v_${eventSeq}/event_main_010.png">
						<span id="cursor"></span>
					</div> -->
...
<script> 
	...
	function fnForm(formId){
		...
		let selectedRadio1 = procForm.querySelector(`input[name="tage"]:checked`);
		if (!selectedRadio1) {
			alert("연령대를 선택해주세요.");
			return;
		} else {
			procForm.querySelector("input[name='age']").value = selectedRadio1.value;
		}
		
		let selectedRadio2 = procForm.querySelector(`input[name="tadd1"]:checked`);
		if (!selectedRadio2) {
			alert("설문을 선택해주세요.");
			return;
		} else {
